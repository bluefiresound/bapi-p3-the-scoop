...
function isValidCommentRequest(commentRequest) {
  if (commentRequest.body.comment) {
    const newComment = commentRequest.body.comment;
    if (newComment.username && newComment.username !== '' &&
      newComment.articleId && newComment.articleId !== '' &&
      newComment.body ) {
        return true;
...
function createComment(url, request) {
  ...
  //console.log("*************************** enter " + Object.getOwnPropertyNames(request.body.comment) );

  if ( isValid(request) && isValidCommentRequest(request) ) {
    const newComment = request.body.comment;

    if ( database.users[newComment.username] &&
      database.articles[newComment.articleId]) {
  ...
  //validate if anything goes wrong
  response.status = 400;
  return response;
}
...
function updateComment(url, request) {
  ...
  const commentId = getCommentIdFromUrl(url);
...
function deleteComment(url, request) {
  ...
  if (database.comments[commentId]) {
    ...
    const usernameCommentIdPos = database.users[username].commentIds.findIndex(
      function (element) {
        return (element === commentId);
      }
    );
    database.users[username].commentIds.splice(usernameCommentIdPos, 1);

    //--- articles
    const articleId = database.comments[commentId].articleId;
    const articlesCommentIdPos = database.articles[articleId].commentIds.findIndex(
      function (element) {
        return (element === commentId);
      }
    );
    database.articles[articleId].commentIds.splice(articlesCommentIdPos, 1);
...
function upvoteComment(url, request) {
  ...
  if (request != null && isValid(request) && request.body.username &&
  database.users[request.body.username] && database.comments[commentId]) {
    const username = request.body.username;
    const comment = database.comments[commentId];

    //check if user is already downvoted, if so, need to switch
    if (comment.downvotedBy.includes(username)) {
        const downvotePos = comment.downvotedBy.findIndex(
          function(element) {
            return element === username;
          }
        );
        comment.downvotedBy.splice(downvotePos, 1);
    }
...
function downvoteComment(url, request) {
  ...
  if (request != null && isValid(request) && request.body.username &&
    ...
    if (comment.upvotedBy.includes(username)) {
        ...
        comment.upvotedBy.splice(downvotePos, 1);
    }
